...
```bash
acp-claw cron add --name 每日简报 --schedule "30 8 * * *" \
  --chat-id <群 chat_id> --fresh-session --prompt "<见下方模板>"
```
...
（先跑 collect 采集素材，再写日报，写入飞书文档，最后把日报正文作为最终回复）。
```
...
2. 写一份**中文**日报，控制在 500 字以内，结构固定：
   ...
   - 📄 论文：按「Token 压缩 / 量化 / Infra」各挑 1–2 篇最有价值的，给一句话点评（为什么值得看）
...
5. **最终回复 = 日报正文**（cron 任务会用 `--chat-id` 把它发到群）；不要输出中间过程、不要复述工具原始输出
