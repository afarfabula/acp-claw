...
### 回复末尾附带本回合花费（`replyCost`）

默认关闭。在 `config.json` 里开启后，每个回合结束会在回复末尾追加一行人民币花费：

```json
{ "replyCost": { "enabled": true } }
```

效果：`💸 本回合 ¥0.078（4 次请求 · 缓存命中 99.5% · 高峰） · 会话累计 ¥12.31（465 次请求）`

- 按 DeepSeek 官方人民币价换算（flash 高峰 ¥2 / ¥8 / ¥0.04 = 未命中输入 / 输出 / 缓存命中，空闲半价），
  峰谷按**每轮事件的时刻**逐轮判定
- 「本回合」= 这一次提问引发的**全部**模型请求之和（一次回答往往是「模型 → 工具 → 模型 → …」多次请求），
  不是最后一次请求的花费；「会话累计」= 该会话至今所有请求之和
- 解析是增量的：每个会话文件记住已读偏移，每回合只读新增部分，长会话也不会变慢
- 数据源是 Codex 会话记录 `$CODEX_HOME/sessions/**/rollout-*.jsonl`（按会话的 thread id 定位，
  可用 `replyCost.codexHome` 覆盖根目录）；算不出来时静默跳过，不影响回复
- `replyCost.channels` 默认 `["feishu"]`，A2A / inject 等通道不会带这行，避免污染协议
